...
def final_adjustments(df,adj_dummies=[]):    
    ...
    dummies = pd.get_dummies(df[['room_type_nan','neighbourhood','neighbourhood_group','room_type']],dummy_na=True)
    df = pd.concat([df,dummies],axis=1)
    df = df.drop(columns=['neighbourhood_group', 'neighbourhood', 'room_type', 'year','year_month','id','host_id'])
