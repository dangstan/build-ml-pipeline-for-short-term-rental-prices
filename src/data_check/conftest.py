...
def data(request):
    ...
    data_path = run.use_artifact(request.config.option.csv).get_path('featurized.csv').download()
...
def columns(request):
    ...
    # Download input artifact. This will also note that this script is using this
    # particular version of the artifact
    data_path = run.use_artifact(request.config.option.columns).get_path('boruta_featured.json').download()
