...
def go(args):
    ...
    run = wandb.init(job_type="train_lgbm")
    ...
    X = X[['last_review','name','price','room_type','neighbourhood_group']+boruta_features]
    ...
    # Compute r2 and MAE
    logger.info(f"Scoring: {len(X)} rows, {len(X.columns)} columns")
